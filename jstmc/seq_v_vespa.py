...
class SeqVespaGerd(seq_baseclass.Sequence):
    def __init__(self, pypsi_params: pypsi.Params = pypsi.Params()):
        ...
        self.block_pf_acquisition, grad_read_pre_area = Kernel.acquisition_pf_undersampled(
        ...
        self._mod_excitation(grad_pre_area=grad_read_pre_area)
    ...
    def _add_gesse_readouts(self, idx_pe_loop: int, idx_slice_loop: int,
        ...
        if no_adc:
            # gre readout
            aq_block_gre = self.block_gre_acq.copy()
            aq_block_gre.adc = events.ADC()
            # se readout
            aq_block_se = self.block_se_acq.copy()
            aq_block_se.adc = events.ADC()
        else:
            aq_block_gre = self.block_gre_acq
            aq_block_se = self.block_se_acq
    ...
    def _loop_slices(self, idx_pe_n: int, no_adc: bool = False):
        ...
        if no_adc:
            ...
            aq_block_pf = self.block_pf_acquisition.copy()
            aq_block_pf.adc = events.ADC()
        else:
            aq_block_pf = self.block_pf_acquisition
        for idx_slice in range(self.params.resolution_slice_num):
            ...
            self._set_phase_grad(phase_idx=idx_pe_n, echo_idx=0, excitation=True)
            # add block
